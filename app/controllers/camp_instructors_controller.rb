class CampInstructorsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @camp_instructor.save
        ...
        format.json { render action: 'show', status: :created, location: @camp_instructor }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to camp_instructors_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def camp_instructor_params
      params.require(:camp_instructor).permit(:id, :camp_id, :instructor_id)
    end
end
